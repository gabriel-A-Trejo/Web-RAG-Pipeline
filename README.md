...
```json
{
  "q": "What happened in AI news this week?"
}
```
...
### `POST /undefined/search 404`

Client env var is missing. Set:
...
Agent `ALLOWED_ORIGIN` must match client URL:

```env
ALLOWED_ORIGIN=http://localhost:3000
```

### GitHub push blocked for secrets

Do not commit `.env` files. Keep keys in local env only and rotate keys if leaked.

## Security Notes

- Never commit API keys.
- `.env` files should stay ignored by git.
- If a key was pushed or exposed, revoke and rotate it immediately.
